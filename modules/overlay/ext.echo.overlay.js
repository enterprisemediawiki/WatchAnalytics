	...
	mw.echo.overlay = {
		...
		buildOverlay: function ( callback ) {
			...
			api.get( mw.echo.desktop.appendUseLang( apiData ) ).done( function ( result ) {
				...
				window.location.href = $( '#pt-watchlist a' ).attr( 'href' );
			} );
		}
	};

	$( function () {
		var $link = $( '#pt-watchlist a' );
		if ( ! $link.length ) {
			return;
		}
		...
		$( 'body' ).click( function ( e ) {
			if ( ! $( e.target ).is( '.mw-echo-overlay, .mw-echo-overlay *, .mw-echo-overlay-pokey, #pt-watchlist a' ) ) {
				mw.echo.overlay.removeOverlay();
			}
		} );
