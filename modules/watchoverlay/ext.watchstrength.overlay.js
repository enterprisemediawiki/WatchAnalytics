...
( function ( $, mw ) {
	...
	mw.watchstrength.overlay = {
		...
		buildOverlayContent : function () {
			var $ul = $( '<ul>' )
				.addClass( 'mw-watchstrength-overlay-content' );

			var $lis = this.buildListElements();
			
			if ( $lis.length )
				$ul.append( $lis );
			else
				$ul.append( this.buildEmptyList() );
    ...
    // <a class="mw-echo-notification-wrapper" href="/wiki/User_talk:Jamesmontalvo3">
    //      <div class="mw-echo-state">
    //           <img class="mw-echo-icon" src="">
    //                <div class="mw-echo-content">

		buildListElements : function () {
			...
			$wrapper = $( '<a>' )
				.addClass( 'mw-watchstrength-notification-wrapper')
				.attr( 'href', notification.href );

			$state = $( '<div>' )
				.addClass( 'mw-watchstrength-state' );

			$icon = $( '<img>' )
				.class( 'mw-watchstrength-icon' )
				.attr( 'src', '//bits.wikimedia.org/static-1.24wmf13/extensions/Echo/modules/icons/Talk.png' );

			$content = $( '<div>' )
				.addClass( 'mw-watchlist-content' );
